...
int nega_alpha_transpose(board b, int depth, bool passed, int alpha, int beta, int cell_score[hw / 2][n_line]) {
    ++visited_nodes;
    ...
    if (canput == 0) {
        ...
        if (passed)
            return evaluate(b, cell_score);
        b.player = 1 - b.player;
        return -nega_alpha_transpose(b, depth, true, -beta, -alpha, cell_score);
    }
    ...
    // 探索
    for (const board& nb: child_nodes) {
        g = -nega_alpha_transpose(nb, depth - 1, false, -beta, -alpha, cell_score);
        if (g >= beta) { // 興味の範囲よりもminimax値が上のときは枝刈り fail high
            if (g > l) {
                // 置換表の下限値に登録
                transpose_table_lower[b] = g;
